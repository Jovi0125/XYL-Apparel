...
{{-- Filters --}}
<div class="mb-6">
    <form method="GET" action="{{ route('seller.orders.index') }}" class="flex flex-wrap gap-3">
        ...
        @if(request('search') || request('status'))
            <a href="{{ route('seller.orders.index') }}" class="px-4 py-2.5 text-gray-500 rounded-lg text-sm hover:text-gray-900 transition">Clear</a>
        @endif
    </form>
</div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4">
                        <a href="{{ route('seller.orders.show', $order) }}" class="font-medium text-gray-900 hover:underline">#{{ $order->order_number }}</a>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-gray-900">{{ $order->customer->name ?? '—' }}</p>
                    </td>
                    <td class="px-6 py-4 text-gray-600">{{ $order->items->count() }}</td>
                    <td class="px-6 py-4 text-gray-900 font-medium">₱{{ number_format($order->total, 2) }}</td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <a href="{{ route('seller.orders.show', $order) }}" class="text-sm text-gray-600 hover:text-gray-900 font-medium transition">View</a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center text-gray-400">No orders yet.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
