...
<div class="max-w-3xl">
    ...
    <form method="POST" action="{{ route('seller.products.update', $product) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent resize-none">{{ old('description', $product->description) }}</textarea>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <h2 class="text-base font-semibold text-gray-900 mb-4">Current Images</h2>
            <div class="grid grid-cols-4 sm:grid-cols-6 gap-3">
                @foreach($product->images as $image)
                <div class="relative group">
                    <img src="{{ asset('storage/' . $image->path) }}" alt="" class="w-full aspect-square rounded-lg object-cover">
                    @if($image->is_primary)
                        <span class="absolute top-1 left-1 px-1.5 py-0.5 bg-gray-900 text-white text-[10px] rounded font-medium">Primary</span>
                    @endif
                    <label class="absolute top-1 right-1 flex items-center gap-1 px-1.5 py-0.5 bg-red-500 text-white text-[10px] rounded font-medium cursor-pointer opacity-0 group-hover:opacity-100 transition">
        ...
        {{-- Upload New Images --}}
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <h2 class="text-base font-semibold text-gray-900 mb-1">Add More Images</h2>
            <p class="text-sm text-gray-400 mb-4">Max 8 images total.</p>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6" x-data="variantManager()">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-sm text-gray-400">Manage size/color variants.</p>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-3 p-3 bg-gray-50 rounded-lg items-end">
                    ...
                    <div>
                        ...
                        <input type="text" :name="'variants['+index+'][size]'" x-model="variant.size"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" placeholder="S, M, L...">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Color</label>
                        <input type="text" :name="'variants['+index+'][color]'" x-model="variant.color"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" placeholder="Black, White...">
                    </div>
                    <div>
                        ...
                        <input type="number" :name="'variants['+index+'][stock]'" x-model="variant.stock" min="0"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" placeholder="0">
                    </div>
                    <div>
                        ...
                        <input type="number" :name="'variants['+index+'][price_override]'" x-model="variant.price_override" step="0.01" min="0"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" placeholder="Optional">
                    </div>
                    <div class="flex items-center">
                        <input type="text" :name="'variants['+index+'][sku]'" x-model="variant.sku"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" placeholder="SKU">
            ...
            <div x-show="variants.length === 0" class="text-sm text-gray-400 text-center py-4">No variants.</div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="checkbox" name="cash_on_delivery" id="cash_on_delivery" value="1" {{ old('cash_on_delivery', $product->cash_on_delivery) ? 'checked' : '' }}
                        class="w-4 h-4 rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                ...
                <div>
                    ...
                    <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $product->video_url) }}"
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="px-6 py-2.5 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition">Update Product</button>
...
function variantManager() {
    return {
        variants: @json($product->variants->map(fn($v) => [
            'id' => $v->id,
            'size' => $v->size,
            'color' => $v->color,
            'stock' => $v->stock,
        ...
        addVariant() {
            this.variants.push({ id: '', size: '', color: '', stock: 0, price_override: '', sku: '' });
        },
        removeVariant(index) {
