    ...
    return (
        <AdminLayout title="Order Management" activeItem="orders">
            ...
            <div className="max-w-[90rem] mx-auto space-y-6">
                ...
                {/* Header Section */}
                <div>
                    <h1 className="text-2xl font-semibold text-white tracking-tight">Order Entries</h1>
                    <p className="text-sm text-slate-400 mt-1">Master list of customer order creations across all channels.</p>
                </div>

                {/* Toolbar */}
                <div className="bg-slate-900/80 p-4 border border-slate-800/50 backdrop-blur-sm rounded-xl flex justify-between items-center shadow-xl">
                    <input 
                        type="text" 
                            ...
                            <tbody className="divide-y divide-slate-800/40">
                                {filteredOrders.length > 0 ? filteredOrders.map(order => (
                                    <tr key={order.id} className="hover:bg-slate-800/20 transition-colors">
                                        <td className="px-5 py-4 font-mono text-xs text-teal-400 bg-teal-500/5 rounded-md inline-block mt-2 ml-4 mb-2">{order.order_number}</td>
                                        <td className="px-5 py-4">
                                            <div className="text-white text-sm">{order.buyer_name}</div>
                                            <div className="text-slate-500 text-[11px]">{order.buyer_email}</div>
                                        </td>
                                        <td className="px-5 py-4 text-slate-300">
                                            {order.product_title} 
                                            <span className="text-slate-500 ml-1">(x{order.quantity})</span>
                                        </td>
                                        <td className="px-5 py-4 text-emerald-400 font-medium text-right">₱{Number(order.total_amount).toLocaleString()}</td>
                                        <td className="px-5 py-4 text-center">
                                            <span className={`px-2 py-1 flex justify-center items-center gap-1 rounded text-[10px] uppercase font-bold w-max mx-auto ${order.payment_status === 'paid' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                                                {order.payment_status}
                                            </span>
                                        </td>
                                        <td className="px-5 py-4 text-slate-400 text-sm">{order.date}</td>
                                    </tr>
                                )) : (
                                    <tr>
