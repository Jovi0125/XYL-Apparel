                            ...
                            {users.length > 0 ? users.map((user) => (
                                <tr key={user.id} className="hover:bg-gray-50 transition">
                                    ...
                                    <td className="px-6 py-4 text-right space-x-3">
                                        <Link to={`/admin/users/${user.id}`} className="inline-flex items-center text-blue-600 hover:text-blue-800" title="View">
                                        ...
                                        {user.is_banned ? (
                                            <button onClick={() => handleUnban(user.id)} className="inline-flex items-center text-green-600 hover:text-green-800" title="Unban">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                </svg>
                                            </button>
                                        ) : (
                                            <button onClick={() => handleBan(user.id)} className="inline-flex items-center text-red-600 hover:text-red-800" title="Ban">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
