...
export default function ReportsProducts() {
    ...
    useEffect(() => {
        axios.get('/seller/reports/products', { params: { period } }).then(res => {
    ...
    return (
        <DashboardLayout sidebar={<SellerSidebar />} pageTitle="Product Performance">
            <div className="flex items-center justify-between mb-6">
                <div className="flex gap-2">
                    <Link to="/seller/reports" className="px-3 py-1.5 border border-gray-200 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-50">Overview</Link>
                    <Link to="/seller/reports/products" className="px-3 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-lg">Products</Link>
                </div>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {products.length > 0 ? products.map((p) => (
                                <tr key={p.id} className="hover:bg-gray-50 transition">
                                    ...
                                    <td className="px-6 py-4 text-right text-gray-600">{p.units_sold}</td>
                                    <td className="px-6 py-4 text-right font-medium text-gray-900">₱{Number(p.revenue).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
